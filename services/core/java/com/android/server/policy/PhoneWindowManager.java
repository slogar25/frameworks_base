...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void wakeUpFromWakeKey(KeyEvent event, boolean withProximityCheck) {
        if (wakeUp(event.getEventTime(), mAllowTheaterModeWakeFromKey,
            ...
            final boolean keyCanLaunchHome = event.getKeyCode() == KeyEvent.KEYCODE_HOME || event.getKeyCode() == KeyEvent.KEYCODE_POWER;
            // Start HOME with "reason" extra if sleeping for more than mWakeUpToLastStateTimeout
            if (shouldWakeUpWithHomeIntent() && keyCanLaunchHome) {
                startDockOrHome(DEFAULT_DISPLAY, /*fromHomeKey*/ event.getKeyCode() == KeyEvent.KEYCODE_HOME, /*wakenFromDreams*/ true,
                        "Wake from " + KeyEvent.keyCodeToString(event.getKeyCode()));
            }
        }
    }
